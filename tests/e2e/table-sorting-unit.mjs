...
const results=await runAll();console.log(formatResults(results));const report=process.env.TEST_REPORT_DIR??'reports/tests/table-sorting';fs.mkdirSync(report,{recursive:true});fs.writeFileSync(`${report}/unit-results.json`,JSON.stringify({results},null,2)+'\n');if(results.some(r=>r.status!=='PASS'))process.exitCode=1;
