...
test('Space sorts all five tables both ways using exact independent values and retains focus',()=>pageTest(async page=>{
 for(const c of tables){await visit(page,c.route.replace('year=all','year=2013')+`&${c.id}_sort=imports&${c.id}_dir=desc`);if(c.id==='home')await page.getByRole('button',{name:/Show all/}).click();const table=page.locator(c.selector),button=table.getByRole('button',{name:/^Imports: sort /});await button.focus();for(const direction of ['asc','desc']){await button.press('Space');assert.equal(await button.evaluate(e=>document.activeElement===e),true);assert.equal(await button.locator('..').getAttribute('aria-sort'),direction==='asc'?'ascending':'descending');const rows=annualRows(c.id,c.code,2013,'I'),expected=expectedOrder(rows,r=>flowValue(r,'imports'),r=>r[c.key],direction).map(r=>String(r[c.key]));assert.deepEqual(await ids(table,c),expected);assert.equal(new URL(page.url()).searchParams.get(`${c.id}_dir`),direction);}}
}));
