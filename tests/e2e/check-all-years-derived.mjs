...
const result={checks,status:'PASS',bytes:bytes.length,sha256:createHash('sha256').update(bytes).digest('hex')};const output=process.argv[3]??path.join(process.env.TEST_REPORT_DIR??'reports/tests/all-years','derived-integrity.json');fs.mkdirSync(path.dirname(output),{recursive:true});fs.writeFileSync(output,JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify(result));
