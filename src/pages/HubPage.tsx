...
export function HubPage(): JSX.Element {
  const search = useSearchParam('hub_q');
  const sectionSearch = useSearchParam('hub_sections_q');
  const query = search.value;
  const sectionQuery = sectionSearch.value;
  const partnerSearchRef = useRef<HTMLInputElement>(null);
  const sectionSearchRef = useRef<HTMLInputElement>(null);
  const clearPartnerSearch = () => {
    search.clear();
    partnerSearchRef.current?.focus();
  };
  ...
  return <div className="hub-page">
    ...
    <section id="hub-partners" className="panel" aria-labelledby="hub-partner-title"><div className="section-heading-row"><div><p className="eyebrow">Choose a trading partner</p><h2 id="hub-partner-title">Partners</h2></div><span className="count-badge">{approved.length} in this snapshot</span></div>
      <div className="field"><label htmlFor="hub-partner-search">Search partners</label><input id="hub-partner-search" ref={partnerSearchRef} type="search" placeholder="Name or Census code" value={query} onChange={(e) => search.setValue(e.target.value)} /></div>
      <p role="status" className="chart-note">Showing {matches.length} of {approved.length} partners.</p>
      {!matches.length && <div className="empty-state"><h3>No partners match “{capQuery(query)}”</h3><p>Try another name or a Census partner code.</p><button type="button" onClick={clearPartnerSearch}>Clear search</button></div>}
    ...
    <section id="hub-sections" aria-labelledby="hub-section-title"><div className="section-heading-row"><div><p className="eyebrow">Harmonized System categories</p><h2 id="hub-section-title">Product groups</h2></div><span className="count-badge">22 groups</span></div>
      <div className="field"><label htmlFor="hub-section-search">Search product groups</label><input id="hub-section-search" ref={sectionSearchRef} type="search" value={sectionQuery} onChange={(e) => sectionSearch.setValue(e.target.value)} placeholder="Product name, HS section or chapter code" /></div>
      <p role="status" className="chart-note">Showing {matchingSections.length} of {sections.data.groups.length} product groups.</p>
      {!matchingSections.length && <div className="empty-state"><h3>No product groups match “{capQuery(sectionQuery)}”</h3><p>Try a different product name, HS section, or chapter code.</p><button type="button" onClick={clearSectionSearch}>Clear product search</button></div>}
