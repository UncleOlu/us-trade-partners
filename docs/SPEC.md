...
## UI RULES
- `year=all` selects the sum of every configured annual year, with the full covered period visible. Default remains the latest single year. Preserve all-years state across home, hub, partner and section links. Trends remain annual. Disable the single-year slider in all-years mode with a clear way to select one year.
- All-years sums use integer USD and must remain safe integers. A missing required year or absent value makes that cumulative flow absent. Structural not_applicable years are excluded, with a visible explanation; all-not_applicable remains not_applicable. Derive balance and total trade only from complete cumulative imports/exports. Sum annual world totals separately, never ranked rows; sum each EU annual historical-membership value only for the EU row. Explain nominal-dollar sums and changing EU membership.
- The home all-years view may use a deterministic build-derived file outside the canonical dataset, generated only from the pinned annual summaries. It must identify snapshot and covered years, pass runtime guards, and fit the existing home budget. It does not change the raw archive, canonical schemas, release pin or pipeline metadata.
- Center rank and monetary headings and cells in the home ranked-partner table. Keep partner names left-aligned and preserve exact-value access, one-line numbers and contained scrolling.
- Use the same centered rank and monetary columns in the section partner table; names and kind stay left-aligned.
- Center years and monetary headings and cells in partner detail tables. Keep product group names, chapter codes and descriptions left-aligned. Preserve exact-value access and contained scrolling.
- Data tables support ascending and descending sorting through keyboard-accessible column headings, with a visible direction and aria-sort. Compare exact numeric values, including negative balances and zero. Missing values remain last in both directions; ties use stable identifiers. Text sorts use names or descriptions; Group order uses canonical HS section order. Keep initial defaults: partner years chronological, groups in HS order, other tables by descending total trade. CSV exports follow the complete sorted table order. Home search and visible limits do not reduce its full CSV scope. Sorting does not change chart data, totals or product selection.
- Save each table's sort choice and direction in its own URL parameters. Keep year, rank and selected group state valid across refresh and navigation. On home and section tables, financial rank remains the descending rank for the selected money metric even when display order changes. Explain display order and keep the first-25 control label accurate for ascending or text sorts.
...
- Partner page: line chart of imports, exports, balance by year. Bar chart of sections by flow for the selected year. Chapter table initially sorted by total_trade_value, with selectable ascending and descending column sorts. Status label for every non-observed value.
