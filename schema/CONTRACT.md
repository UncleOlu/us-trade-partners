...
`year=all` is UI state, not a canonical year identifier. Canonical published files retain their integer-year schemas. The app may derive an all-years view from every year in meta.configured_coverage.years. It sums integer values and rejects unsafe integer results. Missing annual records and absent required values produce an absent cumulative flow; they never imply zero. Structural not_applicable years contribute no value and do not block other applicable years, with a visible explanation. All-not_applicable remains not_applicable. Derived balance and total trade require complete cumulative inputs. World totals sum the annual summary.world fields, not partner rows. EU totals sum only its annual historical-membership values.

A home view built from pinned annual summaries may be served as derived/<snapshot_id>/summary-all.json. This is a deterministic app artifact with explicit all-years identity, covered years and snapshot identity, not an additional canonical dataset file. Do not rewrite the snapshot release or meta.code_commit for this UI calculation. Annual charts retain the original records. Map year state accepts a number or `all`; periodLabel supplies the visible covered period. Geometry and fixed USD bands do not change.

### Canonical annual values
